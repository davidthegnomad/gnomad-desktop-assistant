use crate::agent::settings::{resolve_path_input, TrustMode};
use crate::error::{into_invoke_err, GnomadError};
#[cfg(not(test))]
use crate::config::keychain::get_credential_value;
use hmac::{Hmac, Mac};
...
fn load_or_create_secret() -> Result<Vec<u8>, String> {
    ...
    if let Err(e) = crate::config::keychain::store_credential(KEYCHAIN_PATH_SECRET, &encoded) {
